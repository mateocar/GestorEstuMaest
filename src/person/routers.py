...
def create_student(student: OnlyStudent, db: Session = Depends(get_sessiondb)):
    ...
    db_student = students.create_student(student_name = student.student_name, age = student.age, db = db)
    return db_student
...
def update_student(id: int, student_name: str, age: int, db: Session = Depends(get_sessiondb)):
    updated_student = students.update_students(id = id, student_name = student_name, age = age, db = db)
...
def create_teacher(teacher: OnlyTeacher, db: Session = Depends(get_sessiondb)):
    db_teacher = teachers.create_teacher(teacher_name = teacher.name, area = teacher.area, db = db)
    return db_teacher

@person_router.put("/teacher/{id}", response_model= Teacher)
def update_teacher(id: int, teacher_name: str, area: str, db: Session = Depends(get_sessiondb)):
    updated_teacher = teachers.update_teachers(id= id, teacher_name= teacher_name, area= area, db= db)
