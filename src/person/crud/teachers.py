...
def update_teachers(id: int, teacher_name: str, area: str, db: Session)->Teacher:
    teacher_to_update = db.query(Teacher).filter(Teacher.id == id).first()
    if not teacher_to_update:
        raise exception.teacher_not_exist_handler(status_code = 404, detail = "El profesor no existe")
