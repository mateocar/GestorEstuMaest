...
def update_area(id: int, area_name: str, db: Session) -> models.Area:
    area_to_update = (
        db.query(models.Area).filter(models.Area.id == id).first()
    )  # no es necesario el models.Area.id en el filtro solo poner id=id
...
def remove_area(
    id: int, db: Session
) -> Dict[str, str]:  # la flecha indica que es lo que devuelve la funcion
